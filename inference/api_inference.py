...
output_dir = os.path.join(config['output_dir'], config['round'])
os.makedirs(output_dir, exist_ok=True)

output_file = os.path.join(output_dir, f"{config['model_id']}.json")
...
async def get_batch_response_with_retry(messages, n=1, max_retries=1):
    ...
    for _ in range(n):
        try:
            ...
            try:
                ...
                error_msg = str(e)
                prompt_answers.append(f"Error: {error_msg}")
            ...
            # Skip ReadTimeout errors
            prompt_answers.append("Read Time Out Error!")
            with open(log_path, 'a') as log_file:
                log_file.write(f"ReadTimeout Error for request\n")
                log_file.write("-" * 40 + "\n")
...
async def process_tasks(input_path):
    with open(input_path, 'r') as f:
        dataset = json.load(f)
    if config['start_idx'] is not None and config['end_idx'] is not None:
        dataset = dataset[config['start_idx']:config['end_idx']]
    ...
    for batch in batches:
        messages = generation_prompt(batch, config)
